    ...
    if ($("#MarketView").length > 0) {
        MarketVM = new Vue({
            ...
            computed: {
                ...
                // },
                BookmakerMarkets: function() {
  console.log("Filtering BookmakerMarkets from subMarkets:", this.subMarkets);
  var self = this;
  return _.filter(this.subMarkets, function(item) {
    console.log("Item:", item.marketName, item.marketType);
    if (item.marketName == null) return false;
    ...
    var self = this;
    console.log("Filtering TossMarkets from subMarkets:", this.subMarkets);
    return _.filter(this.subMarkets, function(item) {
        return item.marketType && item.marketType.toUpperCase().includes("TOSS");
                ...
                mounted() {
    // backend se data load hone ke baad
    console.log(MarketVM.subMarkets);  // Ye sab subMarkets array ko show karega
console.log(MarketVM.BookmakerMarkets); // BookmakerMarkets ka computed value
console.log(MarketVM.TossMarkets); // TossMarkets ka computed value
...
if (!(oldStatus === 'suspended' && newStatus === 'open') && oldStatus !== 'closed') {
    ...
    timeout: 60000,
    xhrFields: {
        withCredentials: true   // 👈 yeh add karo
    },
    headers: {
        "Authorization": `Bearer ${token}`   // 👈 Token header add karo
    },
    ...
    error: function (jqXhr, textStatus, errorThrown) {
        ...
        if (jqXhr.status == 401) {
            ...
            let errorText = jqXhr.responseJSON && jqXhr.responseJSON.hasOwnProperty('error') ?
