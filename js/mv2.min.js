...
//     watch: {
//         back(newVal) {
//             this.animateOdds('back', newVal);
//         },
//         lay(newVal) {
//             this.animateOdds('lay', newVal);
//         }
//     },

//     methods: {
//         animateOdds(type, newValue) {
//             const key = type === 'back' ? 'displayBack' : 'displayLay';
...
//             // smooth-blend animation every 100ms
//             this.animationTimer = setInterval(() => {
//                 const oldVal = this[key];
//                 const diff = target - oldVal;

//                 // if small difference, snap to target
...
//                 // smooth step (0.25 = 25% blending per frame)
//                 this[key] = oldVal + diff * 0.25;
...
//             <a class="price-price price-back mb-show" id="agg-back" v-on:click="$emit('betslip', 'B')">
//                 <span class="price-odd">{{ displayBack.toFixed(2) }}</span>
//             </a>

//             <a class="price-price price-lay ml-4 mb-show" id="agg-lay" v-on:click="$emit('betslip', 'L')">
//                 <span class="price-odd">{{ displayLay.toFixed(2) }}</span>
//             </a>
//         </div>
//     `
// });
        ...
        MarketVM = new Vue({
            ...
            computed: {
                ...
                BookmakerMarkets: function() {
                    ...
                    return _.filter(this.subMarkets, function(item) {
                        if (item.marketName == null) return false;
                        return item.marketName === self.bookmakerMarketName
                            || item.marketName.toLowerCase().includes("bookmaker")
                            || item.marketName === "BOOKMAKER"
                            || (item.marketType && item.marketType.toUpperCase() === "BOOKMAKER");
                    });
                },
               ...
               TossMarkets: function () {
                    var self = this;
                    return _.filter(this.subMarkets, function(item) {
                        return item.marketType && item.marketType.toUpperCase().includes("TOSS");
                    });
                },
            ...
            methods: {
                ...
                //     let self = this;
                //     let qs = svc_prefix + '/v1/markets/catalog2/?id=' + marketId;
...
FetchCatalog: function (marketId) {
    ...
    let authToken = localStorage.getItem("token") || getCookie('wexscktoken') || '';
    ...
    $.ajax({
        ...
        if (!catalog) {
            console.error("Catalog data is empty from API");
            return;
        }
        // unwrap agar API { success, data } format mein return kare
        ...
        // Vue Reactivity fix: Pehle data modify karein phir assign karein
        _.each(catalog.subMarkets || [], function(market) {
        ...
        self.Catalog = catalog;
        self.subMarkets = (typeof window._adminFilterSubMarkets === 'function') ? window._adminFilterSubMarkets(catalog.subMarkets || []) : (catalog.subMarkets || []);
        self.WinnersCount = catalog.winners || 0;

        // IDs ko explicit variables mein save karein safety ke liye
        self.activeMarketId = catalog.marketId || marketId;
        self.activeEventId = catalog.eventId;
        ...
        // 4. Polling Logic (Safely start only after IDs are ready)
        // Purane intervals clear karein taake duplicate na hon
        if (self.BookIntervalId) clearInterval(self.BookIntervalId);
        if (self.OrderIntervalId) clearInterval(self.OrderIntervalId);

        // Start live updates if signalR not enabled
        if (!self.signalRFlags.prices) {
            self.ListMarketData();
            self.BookIntervalId = setInterval(self.ListMarketData, 1000);
        }

        // ListOrders call karne se pehle 200ms ka delay dein taake Vue state settle ho jaye
        setTimeout(function() {
            if (self.activeEventId && self.activeMarketId) {
                self.ListOrders();
                self.OrderIntervalId = setInterval(self.ListOrders, 2000);
            }
        }, 200);
        ...
        error: function(jqXHR) {
            console.error("Catalog API Failed: ", jqXHR.status, jqXHR.responseText);
            self.IsLoaded = false;
        }
    });
},
                ListMarketData: function () {
                    ...
                    if (this.Catalog !== null && this.Catalog.hasOwnProperty("marketId")) {
                        ...
                        var token = localStorage.getItem("token") || getCookie('wexscktoken') || '';
                        var url = pricesUrl + "/markets/data?id=" + id;
                        ...
                        $.ajax({
                            ...
                            error: function (jqXhr) {
                                if (jqXhr.status === 401) {
                                    clearInterval(self.BookIntervalId);
                                    location.reload();
                                }
                            }
                        });
                ...
                RenderMarketData: function (MarketData) {
                    // API response unwrap: { success, data } format support
                    if (MarketData && MarketData.success && MarketData.data) {
                        MarketData = MarketData.data;
                    }
                    // marketBooks missing ya empty ho to return
                    if (MarketData === null || MarketData === undefined) return;
                    if (!MarketData.marketBooks || MarketData.marketBooks.length === 0) return;
                    // frontend me RenderMarketData ke start me
 ...
 const newStatus = Book.marketStatus.toLowerCase();
let oldStatus = this.lastStatus ? this.lastStatus.toLowerCase() : null;

if (!oldStatus) {
    // First time
    this.Catalog.status = Book.marketStatus;
} else if (oldStatus === 'suspended' && newStatus === 'open') {
    // Ignore Open after Suspended
    this.Catalog.status = this.lastStatus;
} else if (oldStatus === 'closed') {
    // Market closed, never change
    this.Catalog.status = this.lastStatus;
} else {
...
// Update lastStatus only if not trying to ignore
if (!(oldStatus === 'suspended' && newStatus === 'open') && oldStatus !== 'closed') {
    this.lastStatus = this.Catalog.status;
}
                ...
                UpdateRunnerData: function (Book, Catalog, Pls) {
                    ...
                    for (let i = 0; i < runnersCount; i++) {
                        ...
                        return String(item.id) === String(Catalog.runners[i].selectionId);
                                                           });
                ...
                UpdateRunnersPriceSize: function (runner, soldier) {
                   ...
                   if (!runner.marketId || !runner.marketId.startsWith("1.")) {
                        liqRate = 1;
                    }
                ...
                SortSubMarkets: function () {
                    // Admin filter: pehle disallowed markets hataao
                    var allowed = (typeof window._adminFilterSubMarkets === 'function')
                        ? window._adminFilterSubMarkets(this.subMarkets)
                        : this.subMarkets;

                    var highPriority = _.filter(allowed, function (sm) {
                        return sm.sortPriority == 1;
                    });
                    var lowPriority = _.filter(allowed, function (sm) {
                        return sm.sortPriority != 1;
                    });
                ...
                PlaceOrder: function () {
                    ...
                    // ── BALANCE CHECK ──────────────────────────────────────────
                    // Wallet balance check karo pehle — 0 ya negative ho to bet block
                    var walletEl = document.querySelector('.wallet-balance');
                    var currentBalance = 0;
                    if (walletEl) {
                        var balText = walletEl.textContent.replace(/[^0-9.-]/g, '');
                        currentBalance = parseFloat(balText) || 0;
                    }
                    if (currentBalance <= 0) {
                        this.Order.error = "Insufficient balance. Please add funds to place a bet.";
                    ...
                    const self = this;
                    const token = localStorage.getItem("token") || getCookie('wexscktoken') || '';
                    const url = ordersUrl + '/orders';

                    // FIX: Backend ke format mein map karo
                    // Backend expects: [{ marketId, selectionId, side, price, size, runnerName }]
                    const payload = Orders.map(function(o) {
                        return {
                            marketId:    o.marketId   || o.mid   || '',
                            selectionId: o.selectionId || o.sid  || 0,
                            side:        (o.side || o.bs || 'B').toUpperCase(),
                            price:       parseFloat(o.price || o.mp || o.up || 0),
                            size:        parseFloat(o.size  || o.us || 0),
                            runnerName:  o.runnerName || o.rn || '',
                        };
                    });
                    ...
                    $.ajax({
                        ...
                        success: function (result, textStatus, jqXhr) {
                            // FIX: Naya backend { success, data: { orders, wallet, liable } } return karta hai
                            var orders = [];
                            if (result && result.success && result.data) {
                                orders = result.data.orders || [];
                                // Wallet update karo agar available ho
                                if (result.data.wallet !== undefined) {
                                    var walletEl = document.querySelector('.wallet-balance');
                                    if (walletEl) walletEl.textContent = 'B: ' + Math.floor(result.data.wallet).toLocaleString();
                                }
                                if (result.data.liable !== undefined) {
                                    var liableEl = document.querySelector('.wallet-exposure');
                                    if (liableEl) liableEl.textContent = ' | L: ' + Math.floor(result.data.liable).toLocaleString() + ' |';
                                }
                        ...
                        error: function (jqXhr, textStatus, errorThrown) {
                            ...
                            if (jqXhr.status == 401) {
                                ...
                                // FIX: Naya backend error { success:false, message } format mein return karta hai
                                var errMsg = '';
                                if (jqXhr.responseJSON) {
                                    errMsg = jqXhr.responseJSON.message || jqXhr.responseJSON.error || '';
                                }
                                self.Order["error"] = errMsg || (textStatus + ': ' + errorThrown);
                            }
                        }
                    });
                },
                ...
                OrderToOverVM: function (order) {
                    // FIX: Naya backend { request_id, market_id, selection_id, side, price, size, matched, status, event_name, runnerName } return karta hai
                    var price   = parseFloat(order.price || order.matchedPrice || order.requestedPrice || 0);
                    var matched = parseFloat(order.matched || order.matchedSize || order.ms || 0);
                    var size    = parseFloat(order.size || order.requestedSize || order.us || 0);
                    var side    = (order.side || order.bs || '').toUpperCase();

                    // profit/liable calculate karo
                    var profit = side === 'B' ? (price - 1) * (matched || size) : (matched || size);
                    var liable = side === 'B' ? (matched || size) : (price - 1) * (matched || size);

                    return {
                        id:     String(order.request_id || order.orderId || order.id || ''),
                        rn:     order.runnerName || order.runner_name || order.runner || order.event_name || String(order.selection_id || order.selectionId || ''),
                        mid:    order.market_id  || order.marketId   || order.mid || '',
                        bs:     side,
                        mp:     price,
                        ms:     matched,
                        up:     price,
                        ...
                        liable: parseFloat(liable.toFixed(2)),
                    };
                },
                ...
                ListOrders: function () {
                    ...
                    const authToken = localStorage.getItem("token") || getCookie('wexscktoken') || '';

                    // FIX: Naya backend /orders/event?marketId=... accept karta hai
                    const params = new URLSearchParams({
                        marketId: this.Catalog.marketId,
                        maxResults: 0
                    });

                    // eventId agar available ho to add karo
                    if (this.Catalog.eventId) {
                        params.set('eventId', this.Catalog.eventId);
                    }
                ...
                RenderOrdersPosition: function (result) {
                    // FIX: Naya backend { success, data: { orders } } format mein return karta hai
                    // Old format: result['vOrders'], result['pls']
                    var orders = [];
                    var pls = [];

                    if (result && result.success && result.data) {
                        var rawOrders = result.data.orders || [];
                        // OrderToOverVM se convert karo
                        orders = rawOrders.map(this.OrderToOverVM.bind(this));
                        // PL naya backend mein alag nahi aata — runner PnL runner_pnl se aata hai
                        pls = [];
                    } else if (result && result.vOrders) {
                        // Old format fallback
                        orders = result.vOrders || [];
                        pls = result.pls || [];
                    }

                    this.SyncOrders(this, orders);
                    if (pls.length > 0) {
                        this.MarketPL = pls;
                    }
                },
    ...
    if (!orders || !Array.isArray(orders)) {
        console.warn("⚠️ SyncOrders received non-array:", orders);
        orders = [];
    }
    ...
    orders.slice().reverse().forEach(function (order) {
        self.RenderOrder(order);
                ...
                FetchLinePosition: function (marketId) {
                    var self = this;
                    var authToken = localStorage.getItem("token") || getCookie('wexscktoken') || '';
                    $.ajax({
                        url: svc_prefix + 'orders/LinePosition/?marketId=' + marketId,
                        type: 'GET',
                        dataType: 'json',
                        headers: authToken ? { "Authorization": "Bearer " + authToken } : {},
                        success: function (result) {
                ...
                RenderOrder: function (order) {
                    ...
                    // FIX: Cancelled/Settled orders dono lists se hata do
                    const cancelledStatuses = ['CANCELLED', 'SETTLED', 'VOIDED'];
                    const isCancelled = order.status && cancelledStatuses.includes((order.status || '').toUpperCase());
                    ...
                    if (isCancelled) {
                        // #bets-panel global arrays se bhi hata do
                        try {
                            if (window.UnMatched) window.UnMatched = window.UnMatched.filter(function(o){ return String(o.orderId) !== String(order.id); });
                            if (window.Matched)   window.Matched   = window.Matched.filter(function(o){ return String(o.orderId) !== String(order.id); });
                            if (typeof render === 'function') render();
                        } catch(e) {}
                        return;
                    }

                    // FIX: ms = matched size, us = unmatched/requested size
                    var matchedSize   = parseFloat(order.ms || order.matched || 0);
                    var unmatchedSize = parseFloat(order.us || order.size || 0);

                    if (matchedSize > 0) {
                        self.Matched.unshift(order);
                    }

                    if (unmatchedSize > 0 && matchedSize === 0) {
                    ...
                    // ── #bets-panel SYNC ───────────────────────────────────────
                    // Vue Matched/UnMatched ke saath plain JS arrays bhi sync karo
                    try {
                        var orderId = String(order.id || '');
                        var side    = (order.bs || order.side || '').toUpperCase();
                        var price   = parseFloat(order.mp || order.up || order.price || 0);
                        var size    = parseFloat(order.ms || order.us || order.size || 0);
                        var rn      = order.rn || order.runnerName || order.event_name || '';

                        if (window.UnMatched) window.UnMatched = window.UnMatched.filter(function(o){ return String(o.orderId) !== orderId; });
                        if (window.Matched)   window.Matched   = window.Matched.filter(function(o){ return String(o.orderId) !== orderId; });

                        if (matchedSize > 0) {
                            if (!window.Matched) window.Matched = [];
                            window.Matched.unshift({ orderId: orderId, rn: rn, mp: price, ms: matchedSize, bs: side });
                        }
                        if (unmatchedSize > 0 && matchedSize === 0 || order.status === 'PENDING') {
                            if (!window.UnMatched) window.UnMatched = [];
                            window.UnMatched.unshift({ orderId: orderId, rn: rn, up: price, us: unmatchedSize, bs: side });
                        }
                ...
                ShouldRender: function (order) {
                    ...
                    // FIX: mid ke saath marketId aur market_id bhi check karo (naya backend dono use karta hai)
                    var orderMarketId = order.mid || order.marketId || order.market_id || '';
                    return orderMarketId === marketId || this.SubMarketIds.includes(orderMarketId);
                },
