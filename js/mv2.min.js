...
console.log(MarketVM.TossMarkets); // TossMarkets ka computed value
 ...
 FetchCatalog: function (marketId) {
    ...
    $.getJSON(qs, function (catalog) {
        ...
        // Assign subMarkets for Vue
        self.subMarkets = catalog.subMarkets || [];
        
        // Map odds for each runner in subMarkets
        _.each(self.subMarkets, function(market) {
            _.each(market.runners, function(runner) {
                // Vue reactivity ensures UI updates
                Vue.set(runner, 'odds', runner.odds || 0);
                Vue.set(runner, 'status', runner.status || 'ACTIVE');
            });
        });
        ...
        _.each(self.Catalog.runners, function (runner) {
            ...
            // Vue reactivity for main Catalog runners
            Vue.set(runner, 'odds', runner.odds || 0);
            Vue.set(runner, 'status', runner.status || 'ACTIVE');
        });
